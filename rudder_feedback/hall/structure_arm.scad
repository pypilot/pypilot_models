...
difference() {
 ...
 union() {
    ...
    supports = 5;
    so = length/supports/2*5/4;
    for(x=[0:supports-1]){
    hull() {
        translate([length*(x)/supports, 0, 0])
            cylinder(h=3, r=1.);
        translate([length*(x+.5)/supports, so, 0])
            cylinder(h=3, r=1.);
    }
    hull() {
        translate([length*(x+1)/supports, 0, 0])
            cylinder(h=3, r=1.);
        translate([length*(x+.5)/supports, so, 0])
            cylinder(h=3, r=1.);
    }
    ...
    hull() {
        translate([length*(supports-.5)/supports, so, 0])
            cylinder(h=3, r=1.);
        translate([length*(.5)/supports, so, 0])
            cylinder(h=3, r=1.);
    }
}
